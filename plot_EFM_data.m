...
fig_dir = fullfile(file_dir, "Summary Plots");
sites = ["Cordoba","Manfredi","Pilar","Villa-Carlos-Paz", "Villa-del-Rosario"];
netCDF = true; % true for netCDF files, false for matlab files
...
for d=1:length(days_to_do)
    ...
    [fig,axs] = plot_time_range(cur_day, next_day, file_dir, sites, netCDF);
%     sgtitle({"EFM data without site correction",sprintf("%d/%d/%d - %d/%d/%d",dv1(3),dv1(2),dv1(1),dv2(3),dv2(2),dv2(1))});
    sgtitle({"EFM data with site correction",sprintf("%d/%d/%d - %d/%d/%d",dv1(3),dv1(2),dv1(1),dv2(3),dv2(2),dv2(1))});
